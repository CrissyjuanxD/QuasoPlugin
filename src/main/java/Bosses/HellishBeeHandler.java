...
public class HellishBeeHandler extends BaseBoss implements Listener {
    ...
    public static HellishBeeHandler spawn(JavaPlugin plugin, Location center) {
        ...
        Bee bee = world.spawn(center, Bee.class, b -> {
            ...
            Objects.requireNonNull(b.getAttribute(Attribute.MAX_HEALTH)).setBaseValue(2000);
            b.setHealth(2000);

            Objects.requireNonNull(b.getAttribute(Attribute.MOVEMENT_SPEED)).setBaseValue(0.45);
            Objects.requireNonNull(b.getAttribute(Attribute.FOLLOW_RANGE)).setBaseValue(80);
            Objects.requireNonNull(b.getAttribute(Attribute.SCALE)).setBaseValue(3.5);
    ...
    private void startRegenerationPhase() {
        ...
        regenTask = new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (tick % 40 == 0 && tick > 0) {
                    double max = bee.getAttribute(Attribute.MAX_HEALTH).getBaseValue();
