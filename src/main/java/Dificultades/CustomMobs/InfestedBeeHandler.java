...
public class InfestedBeeHandler implements Listener {
    ...
    private void setupBeeAttributes(Bee bee) {
        ...
        Objects.requireNonNull(bee.getAttribute(Attribute.FOLLOW_RANGE)).setBaseValue(50);
        Objects.requireNonNull(bee.getAttribute(Attribute.MAX_HEALTH)).setBaseValue(500.0);
        Objects.requireNonNull(bee.getAttribute(Attribute.MOVEMENT_SPEED)).setBaseValue(0.95);
        Objects.requireNonNull(bee.getAttribute(Attribute.SCALE)).setBaseValue(3);
        bee.setHealth(500.0);
    ...
    private void executeSummonWardens(Bee bee) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            Objects.requireNonNull(warden.getAttribute(Attribute.SCALE)).setBaseValue(0.4);
            Objects.requireNonNull(warden.getAttribute(Attribute.MAX_HEALTH)).setBaseValue(2.0);
            Objects.requireNonNull(warden.getAttribute(Attribute.ATTACK_DAMAGE)).setBaseValue(8.0);
    ...
    private void executeRegeneration(Bee bee) {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                if (bee.isValid() && !bee.isDead()) {
                    double maxHealth = Objects.requireNonNull(bee.getAttribute(Attribute.MAX_HEALTH)).getBaseValue();
